...
export default function CarDetailClient({ car }: { car: Car }) {
  ...
  const [lastBookingInfo, setLastBookingInfo] = useState({ name: "", email: "", amount: 0 });

  // Calculate total cost and return date based on duration
  useEffect(() => {
    if (bookingData.pickupDate && rentalQuantity > 0) {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Capture data for the modal before resetting
      setLastBookingInfo({
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-32 pb-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-xl border-2 border-gray-200 overflow-hidden mb-6">
              ...
              <div className="relative h-96 bg-gray-100">
                <Image
                  src={validImages[selectedImage] || "/placeholder-car.jpg"}
                  alt={car.name}
                  fill
                  priority
            ...
            {car.videoUrl && (
              <div className="bg-white rounded-xl border-2 border-gray-200 overflow-hidden mb-6">
                <div className="p-4 border-b border-gray-100">
                  <h2 className="font-bold text-gray-900">Car Video</h2>
                </div>
                <video
                  src={car.videoUrl}
                  controls
      ...
      <SuccessModal
        ...
        message={`We have received your request for the ${car.name}. You can pay now to secure your booking instantly.`}
        payUrl={`/pay?amount=${lastBookingInfo.amount}&car=${encodeURIComponent(car.name || 'Car Rental')}&email=${encodeURIComponent(lastBookingInfo.email)}&first_name=${encodeURIComponent(lastBookingInfo.name.split(' ')[0])}&last_name=${encodeURIComponent(lastBookingInfo.name.split(' ').slice(1).join(' '))}`}
      />
