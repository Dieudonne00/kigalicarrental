    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative min-h-[500px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-20">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center justify-center px-8 py-4 bg-[#2563EB] text-white font-bold rounded-lg hover:bg-[#1D4ED8] transition-all"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {(pickupDateFromSearch || returnDateFromSearch || categoryFromSearch) && (
          <div className="mb-8 bg-blue-50 border border-blue-200 rounded-xl p-4 flex flex-wrap items-center gap-3 text-sm text-blue-900">
            <svg className="w-5 h-5 text-blue-600 flex-shrink-0" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
              <path d="M5 13l4 4L19 7" />
            </svg>
            <span className="font-semibold">Showing results for your search:</span>
            {categoryFromSearch && categoryFromSearch !== "all" && (
              <span className="bg-white px-2.5 py-1 rounded-full border border-blue-200 capitalize">{categoryFromSearch}</span>
            )}
            {pickupDateFromSearch && (
              <span className="bg-white px-2.5 py-1 rounded-full border border-blue-200">Pickup: {pickupDateFromSearch}</span>
            )}
            {returnDateFromSearch && (
              <span className="bg-white px-2.5 py-1 rounded-full border border-blue-200">Return: {returnDateFromSearch}</span>
            )}
          </div>
        )}
            ...
            <div className="bg-white rounded-xl border-2 border-gray-200 p-6 sticky top-4">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-[#2563EB] focus:outline-none text-gray-900">
                    {categories.map((cat) => (
                ...
                <div>
                  ...
                  <select value={selectedTransmission} onChange={(e) => setSelectedTransmission(e.target.value)} className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-[#2563EB] focus:outline-none text-gray-900">
                ...
                <div>
                  ...
                  <select value={selectedSeats} onChange={(e) => setSelectedSeats(e.target.value)} className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-[#2563EB] focus:outline-none text-gray-900">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type="range" min={minPrice} max={maxPrice} value={priceRange[0]}
                      ...
                      className="absolute w-full accent-[#2563EB] pointer-events-auto"
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                <p className="text-sm text-gray-600">
                  Showing <span className="font-bold text-[#2563EB]">{filteredCars.length}</span> of{" "}
          ...
          <div className="flex-1">
            ...
            {!loading && filteredCars.length === 0 && (
              <div className="text-center py-12">
                ...
                <button onClick={resetFilters} className="px-6 py-2 bg-[#2563EB] text-white rounded-lg font-bold hover:bg-[#1D4ED8] transition-all">Clear Filters</button>
              </div>
            )}
                ...
                {filteredCars.map((car) => (
                  <Link key={car.id} href={`/cars/${car.id}`} className="group bg-white rounded-xl border-2 border-gray-200 overflow-hidden hover:border-[#2563EB] transition-all hover:shadow-lg">
                    <div className="relative h-32 md:h-48 bg-gray-100 overflow-hidden">
                      ...
                      {car.featured && (
                        <div className="absolute top-1 md:top-2 right-1 md:right-2 bg-[#2563EB] text-white px-1.5 md:px-3 py-0.5 md:py-1 rounded-full text-[9px] md:text-xs font-bold">Featured</div>
                      )}
                    ...
                    <div className="p-3 md:p-6">
                      <h3 className="text-sm md:text-xl font-bold text-gray-900 mb-1 md:mb-2 group-hover:text-[#2563EB] transition-colors line-clamp-1">{car.name}</h3>
                      ...
                      <div className="flex items-baseline justify-between pt-2 md:pt-4 border-t border-gray-200">
                        <div>
                          <span className="text-base md:text-2xl font-bold text-[#2563EB]">${car.dailyRate}</span>
                          <span className="text-[10px] md:text-sm text-gray-600">/day</span>
                        </div>
                        <div className="text-[#2563EB] group-hover:translate-x-1 transition-transform">
            ...
            {!loading && filteredCars.length > 0 && (
              <div className="mt-16 bg-white rounded-xl border-2 border-gray-200 p-8 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Link href="/contact" className="inline-flex items-center justify-center px-8 py-3 bg-[#2563EB] text-white rounded-lg font-bold hover:bg-[#1D4ED8] transition-all">
                  ...
                  <a href="tel:+250787619387" className="inline-flex items-center justify-center px-8 py-3 border-2 border-[#2563EB] text-[#2563EB] rounded-lg font-bold hover:bg-[#2563EB] hover:text-white transition-all">
