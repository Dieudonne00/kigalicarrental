          ...
          <div className="bg-white rounded-xl border-2 border-gray-200 p-6 sticky top-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input type="range" min={minPrice} max={maxPrice} value={priceRange[0]}
                    ...
                    className="absolute w-full accent-[#01B000]" style={{ zIndex: priceRange[0] > maxPrice - 100 ? 5 : 3 }} />
                  <input type="range" min={minPrice} max={maxPrice} value={priceRange[1]}
                    ...
                    className="absolute w-full accent-[#01B000]" style={{ zIndex: priceRange[1] < minPrice + 100 ? 5 : 4 }} />
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <p className="text-sm text-gray-600">
                Showing <span className="font-bold text-[#01B000]">{filteredCars.length}</span> of <span className="font-bold">{initialCars.length}</span> vehicles
              </p>
            </div>
            {hasActiveFilters && (
              ...
              {filteredCars.map((car) => (
                <Link key={car.id} href={`/cars/${car.id}`} className="group bg-white rounded-xl border-2 border-gray-200 overflow-hidden hover:border-[#01B000] transition-all hover:shadow-lg">
                  <div className="relative h-32 md:h-48 bg-gray-100 overflow-hidden">
                    ...
                    {car.featured && <div className="absolute top-2 right-2 bg-[#01B000] text-white px-2 py-0.5 rounded-full text-xs font-bold">Featured</div>}
                  ...
                  <div className="p-3 md:p-6">
                    ...
                    <div className="flex items-baseline justify-between pt-2 md:pt-4 border-t border-gray-200">
                      ...
                      <svg className="w-4 h-4 md:w-6 md:h-6 text-[#01B000] group-hover:translate-x-1 transition-transform" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor"><path d="M9 5l7 7-7 7" /></svg>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          {filteredCars.length > 0 && (
            <div className="mt-16 bg-white rounded-xl border-2 border-gray-200 p-8 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href="/contact" className="inline-flex items-center justify-center px-8 py-3 bg-[#01B000] text-white rounded-lg font-bold hover:bg-[#019500] transition-all">Contact Us</Link>
                <a href="tel:+250788892976" className="inline-flex items-center justify-center px-8 py-3 border-2 border-[#01B000] text-[#01B000] rounded-lg font-bold hover:bg-[#01B000] hover:text-white transition-all">Call +250 788 892 976</a>
              </div>
            </div>
          )}
        </div>
