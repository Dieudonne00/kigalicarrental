      ...
      <div className="max-w-md w-full bg-white  border border-gray-100 p-8 md:p-12 text-center">
        {status === "loading" && (
          <div className="space-y-4">
            <Loader2 className="w-16 h-16 animate-spin text-[#01B000] mx-auto" />
        ...
        {status === "success" && (
          <div className="space-y-6 animate-in fade-in zoom-in duration-500">
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto">
              <CheckCircle className="w-12 h-12 text-[#01B000]" />
            </div>
            <h1 className="text-3xl font-black text-gray-900">Payment Successful!</h1>
            <p className="text-gray-600">
              Thank you for chooses Kigali Car Hire. Your booking reference is <span className="font-bold text-gray-900">{orderDetails?.merchantRef}</span>.
            </p>
            <div className="bg-gray-50 rounded-2xl p-4 text-left space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-400">Tracking ID:</span> <span className="font-medium text-green-600">{orderDetails?.trackingId}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Status:</span> <span className="text-green-600 font-bold uppercase text-[10px] bg-green-50 px-2 py-1 rounded-full border border-green-100">Completed</span></div>
            </div>
            <Link 
              href="/"
              className="block w-full bg-[#01B000] hover:bg-[#019500] text-white py-4 rounded-2xl font-bold transition-all shadow-lg shadow-green-100"
            >
              Return to Homepage
            </Link>
          </div>
        ...
        {status === "failed" && (
          <div className="space-y-6 animate-in fade-in zoom-in duration-500">
            ...
            <div className="flex flex-col gap-3">
                ...
                <Link 
                href="/contact"
                className="text-sm font-bold text-gray-400 hover:text-green-600"
                >
                Contact Support
                </Link>
            </div>
...
export default function CallbackPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-[#01B000] font-bold">Loading...</div>}>
      <CallbackContent />
    </Suspense>
  );
}
