...
export default function BookNowPage() {
  ...
  const [lastBookingInfo, setLastBookingInfo] = useState({ fullName: "", email: "", budget: "" });
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex flex-col gap-3">
              {modalContent.type === "success" && (
                <button
                  onClick={() => {
                    // Robust parsing of budget amount
                    const budgetValue = lastBookingInfo.budget;
                    let amount = "50"; // Default
                    
                    if (budgetValue.includes('+')) {
                      amount = budgetValue.replace('+', '').trim();
                    } else if (budgetValue.includes('-')) {
                      amount = budgetValue.split('-')[1].trim();
                    }

                    const params = new URLSearchParams({
                      amount: amount,
                      car: "Car Rental Request",
                      email: lastBookingInfo.email,
                      first_name: lastBookingInfo.fullName.split(' ')[0],
                      last_name: lastBookingInfo.fullName.split(' ').slice(1).join(' ')
                    });
              ...
              <button
                onClick={() => setShowModal(false)}
                className="w-full px-6 py-3 rounded-lg font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-all"
              >
                {modalContent.type === "success" ? "Close" : "Try Again"}
              </button>
            </div>
          </div>
        </div>
