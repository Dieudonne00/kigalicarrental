...
const WHATSAPP_NUMBER = "250788892976";
    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end">
      {open && (
        <div className="mb-3 w-[320px] sm:w-[360px] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col" style={{ maxHeight: "70vh" }}>
          <div className="bg-[#01B000] px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                    m.role === "user" ? "bg-[#01B000] text-white rounded-br-sm" : "bg-white border border-gray-200 text-gray-800 rounded-bl-sm"
                  }`}
                >
                  {m.text}
                </div>
          ...
          <div className="p-3 border-t border-gray-200 bg-white">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 text-sm border border-gray-300 rounded-full px-4 py-2 focus:outline-none focus:border-[#01B000]"
              />
              <button
                onClick={send}
                disabled={loading}
                className="bg-[#01B000] text-white rounded-full w-10 h-10 flex items-center justify-center flex-shrink-0 hover:bg-[#019500] transition-colors disabled:opacity-50"
            ...
            <a
              ...
              className="mt-2 flex items-center justify-center gap-2 text-sm font-semibold text-[#01B000] hover:underline"
            >
      ...
      <button
        ...
        className="w-14 h-14 rounded-full bg-[#01B000] text-white shadow-xl flex items-center justify-center hover:bg-[#019500] transition-colors"
      >
        {open ? (
