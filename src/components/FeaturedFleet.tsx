      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
          {cars.map((car, index) => (
            <div
              key={car.id}
              className="bg-white rounded-xl overflow-hidden border-2 border-gray-200 hover:border-[#1e3a8a] hover:shadow-md transition-all duration-300 group flex flex-col"
            >
              <div className="relative h-36 sm:h-40 md:h-48 overflow-hidden flex-shrink-0">
                ...
                <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                  {car.hasActiveBooking ? (
                    ...
                    <span className="px-2 py-0.5 bg-[#1e3a8a] text-white rounded-full text-[10px] font-bold">
                      Available
                    </span>
                  )}
                  {car.videoUrl && (
                ...
                <div className="mt-auto">
                  <div className="flex items-center justify-between mb-2 pt-2 border-t border-gray-100">
                    <div className="flex items-baseline gap-0.5">
                      <span className="text-lg md:text-xl font-bold text-[#1e3a8a]">${car.dailyRate}</span>
                  ...
                  <Link
                    href={`/cars/${car.id}`}
                    className="block w-full text-center bg-[#1e3a8a] text-white px-3 py-2 md:py-2.5 rounded-lg text-xs md:text-sm font-bold hover:bg-[#172554] transition-all"
                  >
                    Hire This Car
                  </Link>
                </div>
