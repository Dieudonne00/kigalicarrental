  ...
  useEffect(() => {
    const fetchFeaturedCars = async () => {
      try {
        ...
        // FIX 1: Transform API data to match our component
        const transformedCars = (data.cars || []).map((car: any) => ({
          ...
          // FIX 2: Get image from ANY possible field name
          imageUrl: car.imageUrl || car.image || 
                   (car.images && car.images[0]) || 
                   car.photoUrl || car.photo || 
                   car.img || "/placeholder-car.jpg",
          hasActiveBooking: car.hasActiveBooking,
        }));
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
            {cars.map((car) => (
              ...
              >
                {/* FIX 3: Image with error handling */}
                  ...
                  <div className="mb-2 md:mb-4 pt-2 md:pt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="hidden md:flex flex-col text-right text-xs text-gray-500">
                        {car.weeklyRate && <span>${car.weeklyRate}/week</span>}
                        {car.monthlyRate && <span>${car.monthlyRate}/month</span>}
                      </div>
                    </div>
                  </div>
