...
export default function FeaturedFleet() {
    ...
    const fetchFeaturedCars = async () => {
      try {
        ...
        if (data.cars && Array.isArray(data.cars)) {
          setCars(data.cars);
          // Set initial price range based on fetched cars
          if (data.cars.length > 0) {
            const prices = data.cars.map((car: Car) => car.dailyRate);
  ...
  const { minPrice, maxPrice } = useMemo(() => {
    if (cars.length === 0) return { minPrice: 0, maxPrice: 500 };
  ...
  const hasActiveFilters = selectedCategory !== "all" || selectedTransmission !== "all" || selectedSeats !== "all";
  ...
  return (
    <section className="py-16 md:py-20 bg-[#F8FAFC]">
      ...
      <div className="container mx-auto px-4">
        ...
        {/* FILTERS AND CARS GRID */}
        <div className="max-w-7xl mx-auto">
          {/* FILTERS SECTION */}
          <div className="mb-8 bg-white rounded-xl border-2 border-gray-200 p-4 md:p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div>
                ...
                <select
                  ...
                  className="filter-select w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-[#1E3A8A] focus:outline-none text-gray-900"
                >
                  {categories.map((cat) => (
                ...
                >
                  {transmissions.map((trans) => (
                    <option key={trans} value={trans}>
                      {trans === "all" ? "All Transmissions" : trans.charAt(0).toUpperCase() + trans.slice(1)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-6 pt-6 border-t border-gray-200">
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="mt-3 sm:mt-0 px-4 py-2 border-2 border-gray-300 text-gray-700 rounded-lg font-bold hover:bg-gray-50 transition-all"
                >
                  Clear Filters
                </button>
              )}
          ...
          {filteredCars.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border-2 border-gray-200">
              ...
              <button
                onClick={resetFilters}
                className="px-6 py-2 bg-[#0B1F3A] text-white rounded-lg font-bold hover:bg-[#1E3A8A] transition-all"
              >
                Clear Filters
              </button>
            </div>
